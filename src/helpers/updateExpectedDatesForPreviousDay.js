...
const yesterdayFormatted = formatDateDMY(yesterday)
  ...
  if (habit.repeat_option === 'Ticked Days') {
    ...
    const filtered_completed_dates = Object.keys(completed_dates)
  .filter((dateStr) => completed_dates[dateStr])
  .map((dateStr) => parse(dateStr, 'dd/MM/yyyy', new Date()))
  .filter((date) => isWithinInterval(date, { start: startOfDay(startOfWeek), end: endOfDay(endOfWeek) }));

for (const date of filtered_completed_dates) {
  completedCount++;
}
